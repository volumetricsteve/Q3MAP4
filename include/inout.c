...
// VS - removed some WIN32 stuff
// There's a lot of stuff in here I believe is related to the way q3map2 got integrated into radiant, and passed text (stdout) back to radiant.
// As integration won't be a thing, since we shouldn't even be running with a GUI anyway, there are chunks in here that shouldn't be needed
// why do we need utf8 conversion?  why are we converting from utf8 at all?
